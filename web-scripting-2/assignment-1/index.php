<?php
    ...
    $userFirstName = 'Warren';
    // $userFirstName = $_POST['first-name'];

    // Define stylesheet name and path
    $stylesheetName = 'style.css';
    $stylesheetPath = 'css/' . $stylesheetName;
    ...
    // Calculations for (variable title)
    $userHeightInInches = ($userFeet * $inchesPerFoot) + $userInches;
    $userHeightInCentimeters = $userHeightInInches * $centimetersPerInch;
    $userHeightInMeters = $userHeightInCentimeters / $centimetersPerMeter;
...
<head>
  ...
  <link rel="preload stylesheet" href="<?= $stylesheetPath ?>" as="style" type="text/css">
...
<body>
  ...
  <main class="flex-col gap-1 p-1">
    ...
    <header class="div-media flex-col gap-1">
      ...
      <h1 class="bg-2 c-1 p-1 text-center">WS2 - Assignment 1</h1>

      <div id="hero-image-container" class="h-2"></div>

    </header>

    <section class="bg-2 c-1 p-2 flex flex-col gap-1 text-center">
        <h2>Hello, <?= $userFirstName ?>!</h2>
        <p><?= $userFirstName ?> is <?= $userFeet ?> feet <?= $userInches ?> inches tall.</p>
        <p>Stylesheet Name: <?= $stylesheetName ?></p>
        <p><?= $userFirstName ?> is <?= $userHeightInInches ?> inches tall.</p>
        <p>Centimeters Per Inch (Conversion Rate): <?= $centimetersPerInch ?></p>
        <p><?= $userFirstName ?> is <?= $userHeightInMeters ?> meters tall</p>
    </section>
