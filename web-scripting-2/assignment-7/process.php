<?php
    ...
    $stylesheetExistence = checkExistence('stylesheet');
    $genderExistence = checkExistence('gender');
    $describeYourselfExistence = checkExistence('describe-yourself');
    $rememberMeExistence = checkExistence('remember-me');

    if (isset($_COOKIE['stylesheet'])) {
        $defaultStylesheet = $_COOKIE['stylesheet'];
    }

    if ($firstNameExistence && $lastNameExistence && $studentNumberExistence && $genderExistence && $stylesheetExistence) {
        ...
        $defaultStylesheet = cleanString($_POST['stylesheet']);
    ...
    if ($rememberMeExistence) {
        setcookie('first-name', $firstName, time() + (86400 * 30) + 1,'/','', true);
        setcookie('last-name', $lastName, time() + (86400 * 30) + 1,'/','', true);
        setcookie('gender', $gender, time() + (86400 * 30) + 1,'/','', true);
        setcookie('stylesheet', $defaultStylesheet, time() + (86400 * 30) + 1,'/','', true);
        setcookie('student-number', $studentNumber, time() + (86400 * 30) + 1,'/','', true);
    } else {
        setcookie('first-name', '', time() - 3600,'/','', true);
        setcookie('last-name', '', time() - 3600,'/','', true);
        setcookie('gender', '', time() - 3600,'/','', true);
        setcookie('stylesheet', '', time() - 3600,'/','', true);
        setcookie('student-number', '', time() - 3600,'/','', true);
    }

    if ($describeYourselfExistence && is_array($_POST['describe-yourself'])) {
            ...
            <div id="navigation" class="display-sm bg-2 flex-col-sm gap-1 p-1 flex-grow-1">
                ...
                <ul id="navigation-first-ul" class="flex-col-sm gap-1">
                    ...
                    <li>
                        ...
                        <a href="index.php" title="Home" class="c-1 active" aria-label="Home">Home</a>
...
<main class="flex-col gap-1 p-1">
    ...
    <section class="bg-2 c-1 py-2 px-2 flex flex-col gap-1 text-center">
                <?php
        ...
        if ($firstNameExistence && $lastNameExistence && $studentNumberExistence && $genderExistence && $stylesheetExistence) {
            ...
            echo '<p class="">This page is using the <span class="font-weight-600">'.$defaultStylesheet.'</span> stylesheet.</p>';
        ...
        // Display a link to form/index.php
        echo '<a class="c-1" href="index.php" title="Try Again!">Try the form again..</a>';
            ...
            <nav aria-label="Footer Home Navigation" class="flex-grow-1 flex-col gap-half min-width-150">
                ...
                <ul class="flex-col gap-half">
                    ...
                    <li>
                        ...
                        <a title="Home" href="index.php#" class="c-4">Home</a>
