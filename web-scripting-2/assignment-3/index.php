...
<main class="flex-col gap-1 p-1">
    ...
    <header class="div-media flex-col gap-1">
        ...
        <h1 class="bg-2 c-1 p-1 text-center">WS2 - Assignment 3</h1>

        <div id="hero-image-container" class="h-2"></div>

    </header>

    <section class="bg-2 c-1 p-2 flex flex-col">

        <h2 class="mb-2">Part A):</h2>
        <ul><?php

            $formattedTarget = ucfirst(strtolower($targetName));

            $studentsCount = count($students);

            $inList = false;

            ?><li class="bg-1 mb-half c-2 p-1 justify-between flex flex-row">
                <span class="font-weight-600">Student Count: </span>
                <span><?= $studentsCount ?></span>
            </li>

            <li class="bg-1 mb-half c-2 p-1 justify-between flex flex-row">
                <span class="font-weight-600">Unformatted Target: </span>
                <span><?= $targetName ?></span>
            </li>
            <li class="bg-1 mb-half c-2 p-1 justify-between flex flex-row">
                <span class="font-weight-600">Formatted Target: </span>
                <span><?= $formattedTarget ?></span>
            </li><?php

        for ($i = 0; $i < $studentsCount; $i++) {

            $formattedName = ucfirst(strtolower($students[$i]));

            if ($formattedName === $formattedTarget) {
                $inList = true; ?>
                <li class="bg-1 c-2 p-1 mt-half mb-half justify-between flex flex-row">
                    <span class="font-weight-600">Target Found: </span>
                    <span><?= $formattedTarget ?></span>
                </li>
            <?php } else { ?>
                <li class="py-1 px-half justify-between flex flex-row">
                    <span>Not Found: </span>
            ...
            if ($inList) { ?>
                <li class="bg-1 c-2 p-1 mt-half mb-half justify-between flex flex-row">
                    <span class="font-weight-600">The search name: '<?= $formattedTarget ?>' was found in the list!</span>
                </li>
            <?php } else { ?>
                <li class="bg-1 c-2 p-1 mt-half mb-half justify-between flex flex-row">
                    <span class="font-weight-600">The search name provided is not in our list</span>
                </li>
            <?php }
    ...
    <section class="bg-2 c-1 p-2 flex flex-col">
        ...
        <h2 class="mb-2">Part B):</h2>

        <ul class="flex justify-center flex-row flex-wrap gap-half">
            <?php

            for ($i = $cardsPerHand; $i > 0; $i--) {

                $random = rand($i, count($cardImages));
                if (isset($cardImages[$random])) {

                    ?><li><img src="cards/<?= $cardImages[$random] ?>" alt="Image" draggable="false"></li><?php
                    unset($cardImages[$random]);
                } else {
                    $i++;
                }
            }
    ...
    <section class="bg-2 c-1 p-2 flex flex-col">
        ...
        <h2 class="mb-2">Part C):</h2>

        <ul class="flex flex-row justify-between flex-wrap gap-2"><?php

            $indexes = array_values($capitalCities);
            $keys = array_keys($capitalCities);

            for ($i = 0; $i < count($capitalCities); $i++) {
                ?>

                <li class="px-1">
                    <p>The capital of <span class="font-weight-600"><?= $keys[$i] ?></span> is <span class="font-weight-600"><?= $indexes[$i] ?></span>.</p>
                </li>
    ...
    <section class="bg-2 c-1 p-2 flex flex-col">
        ...
        <ul><?php
            ...
            ?><li><ul class="flex flex-row flex-wrap mb-2 justify-between"><?php

            for ($i = 0; $i < count($collectionOfNumbers); $i++) {


                ?><li><?php
                $current = $collectionOfNumbers[$i];

                ?><p class="mb-2"><?= $current ?></p><?php

                $sumOfAllNumbers += $current;
                ...
                ?></li><?php } ?></ul></li>
            <li class="bg-1 mb-half c-2 p-1 justify-between flex flex-row">
                <span class="font-weight-600">Sum of All Numbers: </span>
                <span><?= $sumOfAllNumbers ?></span>
            </li>
            <li class="bg-1 mb-half c-2 p-1 justify-between flex flex-row">
                <span class="font-weight-600">Count of Even Numbers: </span>
                <span><?= $evenNumbers ?></span>
            </li>
            <li class="bg-1 mb-half c-2 p-1 justify-between flex flex-row">
                <span class="font-weight-600">Count of Odd Numbers: </span>
