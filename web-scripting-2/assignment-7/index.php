<?php
    ...
    if (isset($_COOKIE['first-name'])) {
        $firstName = $_COOKIE['first-name'];
        $rememberMe = 1;
    }

    if (isset($_COOKIE['last-name'])) {
        $lastName = $_COOKIE['last-name'];
    }
    ...
    if (isset($_COOKIE['student-number'])) {
        $studentNumber = $_COOKIE['student-number'];
    }

    if (isset($_COOKIE['stylesheet'])) {
        $defaultStylesheet = $_COOKIE['stylesheet'];
    }
...
<head>
    ...
    <link rel="preload stylesheet" href="<?= $stylesheetDirectory ?><?= $defaultStylesheet ?>" as="style" type="text/css">
...
<main class="flex-col gap-1 p-1">
    ...
    <header class="div-media flex-col gap-1">
        ...
        <h1 class="bg-2 c-1 p-1 text-center">WS2 - Assignment 7</h1>

        <div id="hero-image-container" class="h-2"></div>

    </header>

    <section class="bg-2 c-1 p-1">
        <form method="POST" action="process.php">

            <p class="mb-2">This page is using the <span class="font-weight-600"><?= $defaultStylesheet ?></span> stylesheet.</p>

            <fieldset class="p-1 flex flex-col gap-1">

                <legend>Please complete and submit the form.</legend>

                <label for="first-name">First Name:</label>
                <input type="text" placeholder="First Name" name="first-name" id="first-name" value="<?= $firstName ?>" required>
    
                <label for="last-name">Last Name:</label>
                <input type="text" placeholder="Last Name" name="last-name" id="last-name" value="<?= $lastName ?>" required>
    
                <label for="student-number">Student Number:</label>
                <input type="text" placeholder="Student Number (A0XXXXXXX)" name="student-number" id="student-number" pattern="^[aA]0[0-9]{7}$" value="<?= $studentNumber ?>" required>

                <label for="stylesheet">Select a Stylesheet:</label>
                <select name="stylesheet" id="stylesheet">
                    <option value="<?= $defaultStylesheet ?>"><?= $defaultStylesheet ?></option>
                    <?php
                        ...
                        if ( count($y) > 2 ) {
                            for ( $i = 2; $i < count($y); $i++ ) {
                                if ( $y[$i] != 'normalize-fwd.css' && $y[$i] !== $defaultStylesheet) {
                ...
                <fieldset>
                    ...
                    <input type="radio" name="gender" id="male" value="male" <?= 'male' === $gender ? 'checked' : '' ?> required>
                    <label for="male">Male</label>

                    <input type="radio" name="gender" id="female" value="female" <?= 'female' === $gender ? 'checked' : '' ?> required>
                    <label for="female">Female</label>
                    
                    <input type="radio" name="gender" id="non-binary" value="non-binary" <?= 'non-binary' === $gender ? 'checked' : '' ?> required>
                ...
                <fieldset>
                    ...
                    <div class="flex">
                        <input type="checkbox" value="healthy" id="healthy" name="describe-yourself[]">
                        <label for="healthy">Healthy</label>
                    </div>
                    <div class="flex">
                        <input type="checkbox" value="wealthy" id="wealthy" name="describe-yourself[]">
                        <label for="wealthy">Wealthy</label>
                    </div>
                    <div class="flex">
                        <input type="checkbox" value="wise" id="wise" name="describe-yourself[]">
                        <label for="wise">Wise</label>
                    </div>
                </fieldset>

                <fieldset>
                    <legend>Want to be remembered?</legend>
                    <div class="flex">
                        <label for="remember-me">Remember Me:</label>
                        <input type="checkbox" name="remember-me" value="remember-me" id="remember-me" <?= $rememberMe ? 'checked' : '' ?>>
                    </div>
                </fieldset>

                <div class="flex flex-row w-full justify-center">
                    <button type="submit" class="w-third">Submit</button>
                </div>
            </fieldset>
        </form>
    </section>
